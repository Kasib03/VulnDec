# cnn_bilstm_codebert_multiclass.py (improved model complexity + class balancing)
...
# -------- LOAD AND EMBED DATA (LIMIT TO 5000) --------
X, y = [], []
...
# -------- CNN + BiLSTM (Enhanced) --------
print("Building enhanced CNN+BiLSTM model...")
model = Sequential()
model.add(Conv1D(filters=128, kernel_size=5, activation='relu', input_shape=(MAX_LEN, EMBED_DIM)))
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=[m[0].replace("plain_", "").replace(".json", "") for m in MODELS],
            yticklabels=[m[0].replace("plain_", "").replace(".json", "") for m in MODELS])
...
# -------- SAVE MODEL --------
model.save("model_multiclass_cnn_bilstm_codebert_blocks_5klimit.keras")
print("Model saved as model_multiclass_cnn_bilstm_codebert_blocks_5klimit.keras")
